...
def main():
    ...
    for event in longpoll.listen():
        if event.type == VkEventType.MESSAGE_NEW:
            if event.text == "шизик добавь меня":
            ...
            if event.text == "шизик ping":
                vk.messages.send(peer_id=event.peer_id, message="pong") 
            elif event.text == "шизик помощь":
                    ...
                    if shizasent == "" or shizasent == " ":
                        ...
                        if event.text == "шизик шизофазия":
                            ...
                            try:
                                ...
                                pass
                        if event.text == "шизик шизофазия вслух":
            ...
            elif event.text == "шизик выпей амнезиак":
            ...
            elif event.text == "шизик ебани себе по голове":
                if event.user_id == "320502491":
                    ...
                    print(event.user_id)
                    vk.messages.send(peer_id=event.peer_id, message="иди нахуй!!!")
            elif "шизик забудь:" in event.text:
                if event.user_id == "320502491":
                    vk.messages.send(peer_id=event.peer_id, message="что это значит?")
                    loadwordlist = open("shiza.txt", "r+")
                    prikolwordlist = loadwordlist.read()
                    loadwordlist.close()
                    prikolwordlist = prikolwordlist.replace(event.text.replace("шизик забудь: ", ""), "")
                    editwordlist = open("shiza.txt", "w+")
                    editwordlist.write(prikolwordlist)
                    editwordlist.close()
                else:
                    print(event.user_id)
                    vk.messages.send(peer_id=event.peer_id, message="иди нахуй!!!")
            elif "шизик убить " in event.text:
                usercmd = event.text.replace("шизик убить ", "")
                if event.user_id == "320502491":
                    ...
                    vk.messages.send(peer_id=event.peer_id, message="иди нахуй!!!")
            elif "шизик воскреси " in event.text:
                usercmd = event.text.replace("шизик воскреси ", "")
                if event.user_id == "320502491":
                ...
                if str(event.user_id) in banusers or event.user_id == "441815332":
                    print(event.user_id + " ignore!")
                else:
                    try:
                        prikoltexta = event.text.split(" ")
                        ...
                        if len(prikoltexta) < 500:
                            ...
                            savedtext = savedtext.replace(",", " ")
                            loadwordlist = open("shiza.txt", "w")
                            loadwordlist.write(savedtext + event.text + " END ")
                            loadwordlist.close()
                        else:
